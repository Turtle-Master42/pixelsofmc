...
public class ItemColorHandler {
    ...
    public static void registerItemColor(RegisterColorHandlersEvent.Item event) {
        ...
        event.getItemColors().register((stack, index) -> index == 1 ? ((BigBucket) stack.getItem()).getColor(stack) : -1, POMitems.TITANIUM_BUCKET.get());
        event.getItemColors().register((stack, index) -> index == 1 ? ((BigBucket) stack.getItem()).getColor(stack) : -1, POMitems.REINFORCED_BUCKET.get());
    }
}
