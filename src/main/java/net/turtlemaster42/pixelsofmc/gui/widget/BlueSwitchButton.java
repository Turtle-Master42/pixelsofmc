...
public class BlueSwitchButton extends Button {
    private static final ResourceLocation TEXTURE =
            new ResourceLocation(PixelsOfMc.MOD_ID, "textures/gui/jei/widgets.png");
    private boolean on;


    public BlueSwitchButton(int pX, int pY, OnPress pOnPress) {
    ...
    public void renderButton(PoseStack pPoseStack, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        BlueSwitchButton.Icon switchButton$icon;
        if (this.isHoveredOrFocused()) {
            switchButton$icon = this.on ? Icon.ON_HOVER : Icon.OFF_HOVER;
        } else {
            switchButton$icon = this.on ? BlueSwitchButton.Icon.ON : BlueSwitchButton.Icon.OFF;
        }
