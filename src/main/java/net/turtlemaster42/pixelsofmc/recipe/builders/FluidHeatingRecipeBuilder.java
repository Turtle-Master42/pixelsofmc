...
public class FluidHeatingRecipeBuilder extends POMRecipeBuilder {
    private FluidStack inputFluid;
    private FluidStack outputFluid;
    private final int energy;

    public FluidHeatingRecipeBuilder(int energy) {
        this.inputFluid = FluidStack.EMPTY;
        this.outputFluid = FluidStack.EMPTY;
        this.energy = energy;
    }

    public static FluidHeatingRecipeBuilder build(int energy) {
        return new FluidHeatingRecipeBuilder(energy);
    }

    public FluidHeatingRecipeBuilder input(Fluid fluid, int amount) {
        this.inputFluid = new FluidStack(fluid, amount);
        return this;
    }

    public FluidHeatingRecipeBuilder output(Fluid fluid, int amount) {
    ...
    public void finish(Consumer<FinishedRecipe> consumer, POMrecipeProvider provider) {
        this.unlockedBy("", ANY_CRITERION).save(consumer, provider.toRL("fluid_heating/" + outputFluid.getFluid().getFluidType().toString().split(":")[1]));
    }
    ...
    public static class Result extends POMRecipeResult {
        ...
        public Result(ResourceLocation pId, FluidStack pInFluid, FluidStack pOutFluid, int pEnergy, Advancement.Builder pAdvancement) {
            super(FluidHeatingRecipe.Serializer.INSTANCE, pId, pAdvancement);
