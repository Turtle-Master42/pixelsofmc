...
public class FluidSuperHeatingRecipeBuilder extends POMRecipeBuilder {
    private FluidStack inputFluid;
    private FluidStack outputFluid;
    private final int energy;

    public FluidSuperHeatingRecipeBuilder(int energy) {
        this.inputFluid = FluidStack.EMPTY;
        this.outputFluid = FluidStack.EMPTY;
        this.energy = energy;
    }

    public static FluidSuperHeatingRecipeBuilder build(int energy) {
        return new FluidSuperHeatingRecipeBuilder(energy);
    }

    public FluidSuperHeatingRecipeBuilder input(Fluid fluid, int amount) {
        this.inputFluid = new FluidStack(fluid, amount);
        return this;
    }

    public FluidSuperHeatingRecipeBuilder output(Fluid fluid, int amount) {
    ...
    public void finish(Consumer<FinishedRecipe> consumer, POMrecipeProvider provider) {
        this.unlockedBy("", ANY_CRITERION).save(consumer, provider.toRL("fluid_super_heating/" + outputFluid.getFluid().getFluidType().toString().split(":")[1]));
    }
    ...
    public static class Result extends POMRecipeResult {
        ...
        public Result(ResourceLocation pId, FluidStack pInFluid, FluidStack pOutFluid, int pEnergy, Advancement.Builder pAdvancement) {
            super(FluidSuperHeatingRecipe.Serializer.INSTANCE, pId, pAdvancement);
